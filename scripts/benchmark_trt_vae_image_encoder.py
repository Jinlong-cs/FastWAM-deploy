...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Benchmark a TensorRT FastWAM VAE image encoder engine on AGX.")
    parser.add_argument("--engine", type=Path, default=OUTPUTS_DIR / "trt/vae_image_encoder_fp16.engine")
    parser.add_argument("--output", type=Path, default=OUTPUTS_DIR / "trt/vae_image_encoder_trt_runtime_fp16.json")
...
def make_input_tensor(dtype: torch.dtype, shape: tuple[int, ...], device: torch.device, seed: int) -> torch.Tensor:
...
def benchmark_engine(args: argparse.Namespace) -> dict[str, object]:
    ...
    for idx in range(engine.num_io_tensors):
        ...
        if mode == trt.TensorIOMode.INPUT:
            ...
            tensor = make_input_tensor(dtype=torch_dtype, shape=shape, device=device, seed=args.seed)
        else:
    ...
    output = tensors.get("first_frame_latents")
    output_finite = bool(torch.isfinite(output).all().item()) if output is not None and output.is_floating_point() else None
    output_shape = list(output.shape) if output is not None else None

    return {
        "runtime_mode": "tensorrt_vae_image_encoder",
        "backend": "tensorrt",
        ...
        "tensors": tensor_meta,
        "note": "This benchmark covers only the TensorRT VAE first-frame image encoder partition, not full FastWAM end-to-end inference.",
    }
